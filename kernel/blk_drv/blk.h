 ...
 *
 * 32项好像是一个合理的数字：该数已经足够从电梯算法中获得好处，但当缓冲区
 * 在队列中而锁住时又不显得是很大的数。64就看上去太大了(当大量的写/同步操作
 * 运行时很容易引起长时间的暂停)
 */
 ...
 *
 * OK，下面是request结构的一个扩展形式，因而当实现以后，我们就可以在分页请求中
 * 使用同样的request结构，在分页处理中，'bh'是NULL，而'waiting'则用于等待读/写
 * 的完成.
 *
 * 下面是请求队列中项的结构，其中如果字段dev=-1，则表示队列中该项没有被使用。
 * 字段cmd可取常量READ(0)或WRITE(1)(定义在include/linux/fs.h中)
 */
struct request {
 ...
 * 这是很自然的：读操作对时间的要求要比写操作严格的多。
 *
 * 下面宏中参数s1和s2的取值是上面定义的请求结构request的指针。该宏定义用于根据两个
 * 参数指定的请求项结构中的信息(命令cmd(READ或WRITE))、设备号dev以及所操作的扇区号sector)
 * 来判断出两个请求项结构的前后排列顺序。这个顺序将用作访问块设备时的请求项执行顺序。
 * 这个宏会在程序blk_drv/ll_rw_blk.c中函数add_reqeust()中被调用。该宏部分地实现了I/O调度
 * 功能，即实现乐对请求项的排序功能(另一个是请求项合并功能)
 */
...
/* 设备数据块总数指针数组，每个指针项指向指定主设备号的总块数数组hd_sizes[]。
...
/* 为了便于编程表示，这里定义了两个宏：CURRENT是指定主设备号的当前请求结构项指针
...
#endif
/* 如果定义了设备超时符号常数，则令其值等于0，并定义为SET_INTR()宏，否则只定义宏 */
...
#endif
/* 声明设备请求符号常数DEVICE_REQUEST是一个不带参数并无返回的静态函数指针 */
static void (DEVICE_REQUEST)(void);

/* 解锁指定的缓冲块
 * 如果指定缓冲块bh并没有被上锁，则显示警告信息。否则将该缓冲块解锁，并唤醒等待该缓冲
 * 块的进程。此为内联函数。参数是缓冲块头指针 */
...
/* 结束请求处理
 * 参数uptodate是更新标志
 * 首先关闭指定块设备，然后检查此次读写缓冲区是否有效，如果有效则根据参数值设置缓冲区
 * 数据更新标志，并解锁该缓冲区，如果更新标志参数是0，表示此次请求项的操作已失败，因此
 * 显示相关块设备IO错误信息。最后，唤醒等待该请求项的进程以及等待空闲请求项出现的进程，
...
/* 如果定义了设备中断符号常量DEVICE_INTR，则定义CLEAR_DEVICE_INTR符号常量为"DEVICE_INTR=0"
 * 否则定义为空 */
#ifdef DEVICE_INTR
...
/* 定义初始化请求项宏
 * 由于几个块设备驱动程序开始处对请求项的初始化操作相似，因此这里为他们定义了一个统一的初
 * 始化宏。该宏用于对当前请求项进行一些有效性判断。所做工作如下：如果设备当前请求项为空(NULL)，
 * 表示本设备目前已无需要处理的请求项。于是略做扫为工作就退出相应函数。否则，如果当前请求项中
 * 设备的主设备号不等于驱动程序定义的主设备号，说明请求队列项队列乱掉了，于是内核显示出错信息
 * 并停机。否则若请求项中用的缓冲块没有被锁定，也说明内核程序出了问题，于是显示出错信息并停机 */
#define INIT_REQUEST \
repeat: \
